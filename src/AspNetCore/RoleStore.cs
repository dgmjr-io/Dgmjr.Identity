...
public class RoleStore<TKey, TUser, TRole>(IIdentityDbContext<TUser, TRole> db,
  ...
  protected async Task<MSIDR>
  ProtectedCreateAsync(TRole role, CancellationToken cancellationToken) {
    await Db.Roles.AddAsync(role, cancellationToken);
    await Db.SaveChangesAsync(cancellationToken);
    return IdentityResult.Success("Role created successfully.");
  }

  public async Task<MSIDR> UpdateAsync(TRole role,
  ...
  public async Task<MSIDR> DeleteAsync(TRole role,
                                       CancellationToken cancellationToken) {
  ...
  async Task IRoleStore<TRole>.SetRoleNameAsync(
      TRole role, string? roleName, CancellationToken cancellationToken) {
    await SetRoleNameAsync(role, roleName, cancellationToken);
  }

  public async Task<MSIDR>
  SetRoleNameAsync(TRole role, string roleName,
                   CancellationToken cancellationToken) {
  ...
  public Task<string?>
  GetNormalizedRoleNameAsync(TRole role, CancellationToken cancellationToken) {
    return Task.FromResult<string?>(role.NormalizedName);
  }

  async Task IRoleStore<TRole>.SetNormalizedRoleNameAsync(
      TRole role, string? normalizedName, CancellationToken cancellationToken) {
    (role as IdentityRole).NormalizedName = normalizedName;
    await Db.SaveChangesAsync(cancellationToken);
  }

  public async Task<MSIDR>
  SetNormalizedRoleNameAsync(TRole role, string normalizedName,
                             CancellationToken cancellationToken) {
    (role as IdentityRole).NormalizedName = normalizedName;
    await Db.SaveChangesAsync(cancellationToken);
  ...
  public async Task<TRole?> FindByIdAsync(string roleId,
                                          CancellationToken cancellationToken) {
    return await Db.Roles.FindAsync(new object[] { roleId }, cancellationToken)
        as TRole;
  }

  public async Task<TRole?>
  FindByNameAsync(string normalizedRoleName,
                  CancellationToken cancellationToken) {
    return await Db.Roles.FirstOrDefaultAsync(
        r => r.NormalizedName == normalizedRoleName, cancellationToken);
  }

  public async Task<TRole?>
  FindByUriAsync(uri uri, CancellationToken cancellationToken) {
    return await Db.Roles.FirstOrDefaultAsync(r => (r as IHaveAuri).Uri == uri,
                                              cancellationToken);
  }

  public async Task<MSIDR> CreateAsync(TRole role,
                                       CancellationToken cancellationToken) {
  ...
  // // Override finalizer only if 'Dispose(bool disposing)' has code to free
  // unmanaged resources ~RoleStore()
  // {
