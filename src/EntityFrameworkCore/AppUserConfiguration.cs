...
public class AppUserConfiguration<TUser, TRole, TKey, TUserClaim, TUserRole,
                                  TUserLogin, TRoleClaim, TUserToken,
                                  TClaimType, TClaimValueType>
  ...
  public void Configure(EntityTypeBuilder<TUser> builder) {
    builder.ToTable(TableNames.User, IdentitySchema.ShortName, tb => {
      ...
      tb.HasPhoneNumberCheckConstraint(
          ...
          DataSchema.ShortName);
      tb.HasGenderCheckConstraint(nameof(AppUser.Gender),
                                  ufn_ +
                                      nameof(IdentityDbContext.IsValidGender),
                                  IdentitySchema.ShortName);
      // tb.HasCheckConstraint(
      //     CK_ + nameof(AppUser.EmailAddress),
      //     $"[data].{ufn_ +
      //     nameof(IdentityDbContext.IsValidEmailAddress)}([{nameof(AppUser.TelegramUsername)}])
      //     = 1)"
      // );
      // tb.HasCheckConstraint(
      //     CK_ + nameof(AppUser.PhoneNumber),
      //     $"[{nameof(AppUser.PhoneNumber)}] IS NULL OR
      //     [{nameof(AppUser.PhoneNumber)}] LIKE '+%'"
      // );
      // tb.HasCheckConstraint(
      ...
      //     OR (
      //             len([{nameof(AppUser.TelegramUsername)}]) >=
      //             {Account.UsernameMinLength} AND
      //             len([{nameof(AppUser.TelegramUsername)}]) <=
      //             ({Account.UsernameMaxLength} + 1) AND
      //             [{nameof(AppUser.TelegramUsername)}] LIKE '@%'
      //         )
      //     """
      // );
      // tb.HasCheckConstraint(
      ...
      //     $"{ufn_ +
      //     nameof(IdentityDbContext.IsValidGender)}({nameof(AppUser.Gender)})
      //     = 1"
      // );
      tb.IsTemporal();
    });
    ...
    builder.HasIndex(e => e.EmailAddress)
        .HasDatabaseName(ix_ + TableNames.User +
                         IdentityConstants.User.Columns.EmailAddress);
    builder.HasIndex(e => e.Username)
        .HasDatabaseName(ix_ + TableNames.User +
                         IdentityConstants.User.Columns.Username);
    builder.HasIndex(e => e.TelegramUsername)
        .HasDatabaseName(ix_ + TableNames.User +
                         IdentityConstants.User.Columns.TelegramUsername);
    builder.Property(e => e.TelegramUsername)
        .HasMaxLength(Account.UsernameMaxLength)
        .IsUnicode(false);
    builder.HasIndex(e => e.NormalizedEmailAddress)
        .HasDatabaseName(ix_ + TableNames.User +
                         nameof(AppUser.NormalizedEmailAddress));
    builder.HasIndex(e => e.NormalizedUsername)
        .HasDatabaseName(ix_ + TableNames.User +
                         IdentityConstants.User.Columns.NormalizedUsername);
    builder.Property<byte[]>(IdentityConstants.User.Columns.ConcurrencyStamp)
        .HasColumnType(RowVersion.ShortName)
        .IsRowVersion();
    builder.HasMany(e => e.Roles)
        .WithMany(e => e.Users)
        .UsingEntity<TUserRole>(ur => ur.HasOne(ur => ur.Role)
    ...
    builder.Property(e => e.LockoutEnd)
        .HasDefaultValueSql($"'{AppUser.DefaultLockoutEnd}'");
...
public class AppUserConfiguration
    : AppUserConfiguration<AppUser, AppRole, long, AppUserClaim, AppUserRole,
                           AppUserLogin, AppRoleClaim, AppUserToken,
                           AppClaimType, AppClaimValueType> {}
